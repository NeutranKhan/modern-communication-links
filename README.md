...
In Admin > Content, Gallery, Trainers, and Testimonials have separate forms and save buttons. For gallery photos: Add gallery photo, upload an image, enter a caption, then click Publish gallery. A preview and green upload-success message confirm storage; a separate publication-success message confirms the image is saved to the website. Draft or incomplete testimonials do not block gallery publishing. Remove photo immediately removes a published image from the public gallery after confirmation; no Publish gallery step is required for deletion. Unfinished photo drafts do not block removal. A failed removal keeps the photo visible with an error. If another administrator changed the gallery, reload before retrying. Trainer images use Save trainers. For certificates, open Admin > Participants > Manage, upload the file, then Issue/Save certificate. Image URL fields are hidden; previously stored image links still render. Uploading alone does not publish a photo or issue a certificate.
...
Content-section saves validate only the selected section and merge only that field into content/site, preserving other sections. Empty arrays intentionally clear a section. New content documents are seeded with defaults. Upload and publication failures keep unsaved edits available for retry.
