import {blobOptions} from '@/lib/blob-config';
...
export async function POST(req:NextRequest){try{
 ...
 try{
  const blob=await put((kind==='certificate'?'certificates/':'images/')+randomUUID()+'.'+file.extension,file.bytes,{access:'public',contentType:file.contentType,...storage,addRandomSuffix:true});
