...
export function ContentEditor({initial,user,onSaved}:{initial:Content;user:User;onSaved:(patch:Partial<Content>)=>void}){
 ...
 <form className="form-card" onSubmit={e=>save('gallery',e)}><fieldset className="upload-fieldset" disabled={disabled}><h2>Gallery</h2><p>Add a photo, upload its image, enter a caption, then publish your gallery.</p>
 {content.gallery.map((g,i)=><div className="program-editor" key={i}><h3>Photo {i+1}</h3><FileUpload value={g.url} onBusy={setUploading} successMessage="Image uploaded successfully. Add a caption and click Publish gallery to show it on the site." onUploaded={url=>change('gallery',items=>items.map((item,j)=>i===j?{...item,url}:item))}/><label>Caption / image description<input required maxLength={200} value={g.caption} onChange={e=>change('gallery',items=>items.map((item,j)=>i===j?{...item,caption:e.target.value}:item))}/></label><button className="text-button" type="button" onClick={()=>change('gallery',items=>items.filter((_,j)=>j!==i))}>Remove photo</button></div>)}
 <button type="button" className="button secondary small" disabled={disabled||content.gallery.length>=40} onClick={()=>change('gallery',items=>[...items,{url:'',caption:''}])}>Add gallery photo</button>
